...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This precisobidsmartadvertising no longer exists.'));
        ...
        $this->_coreRegistry->register('precisobidsmartadvertising', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit PrecisoBidSmartAdvertising') : __('New PrecisoBidSmartAdvertising'),
            $id ? __('Edit PrecisoBidSmartAdvertising') : __('New PrecisoBidSmartAdvertising')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('PrecisoBidSmartAdvertising'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New PrecisoBidSmartAdvertising'));
        return $resultPage;
    }
}
